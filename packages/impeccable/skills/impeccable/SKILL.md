...
1. Load project context once per session: read `PRODUCT.md` and (when present) `DESIGN.md` at the project root. If you've already read them in this conversation, don't re-read. **If `PRODUCT.md` is missing, stop and follow `reference/init.md` before doing anything else.** (The `/impeccable` command loads this for you when invoked.)
...
5. **If the project is brand-new (no existing CSS tokens / theme / committed brand colors found in step 3)**, pick a brand seed color before anything else, and treat it as the anchor for your primary brand color. Don't reach for the saturated AI defaults (purple/indigo, the cream/sand/beige band — see the Color rules below). Compose the rest of the palette (bg, surface, ink, accent, muted) around the seed. Use OKLCH throughout. **Skip this step only if step 3 found committed brand colors in existing tokens; in that case identity-preservation wins.**
...
1. **No argument**: the user is asking "what should I do?" Make the menu context-aware instead of static. If `PRODUCT.md` is missing you are already in init (setup), so finish that and skip this. Otherwise read the project signals yourself — `PRODUCT.md` / `DESIGN.md` presence, recent critique snapshots under `.impeccable/critique/`, `git status` for the changed surface — then lead with the **2-3 highest-value next commands**, each with a one-line reason, followed by the full menu (the table above, grouped by category). **Never auto-run a command; the recommendation is a suggestion the user confirms.**

   Reason over the signals; there is no score to obey:
   - DESIGN.md missing while code exists → `document` (capture the visual system).
   - No critique snapshot under `.impeccable/critique/` → the project has never been critiqued; for a set-up project with a real surface, offering `/impeccable critique <surface>` is a strong default.
   - A critique snapshot with a low score or open P0 / P1 items → `polish` (it reads that snapshot as its backlog), or re-run `critique` if the snapshot looks stale.
   - `git status` pointing at one surface → scope `audit` or `polish` to those files specifically, naming them.
   - Otherwise group by intent exactly as init's "Recommend starting points" step does (build new / improve what's there / iterate visually), tailored to the register.

   **Run the `impeccable_detect` tool over the changed markup/style files (or a named surface) once** — it runs the bundled deterministic rules locally (no network). Fold the hits into your picks: many quality / contrast hits → `audit` or `polish`; a specific slop family → the matching command (gradient text or eyebrows → `quieter` / `typeset`, flat or gray palette → `colorize`, and so on). If it errors or the tree is large, skip it and recommend the user run `audit` themselves; never block the suggestion on it.
...
This port has no bundled browser server. For rendered-page work — visual verification, in-browser critique, responsive checks — use pi's native web tools: `web_screenshot` to capture a surface, `web_visit` to read rendered HTML, `web_console` for runtime errors, and `web_interact` to drive the page. To run the deterministic rules against a rendered page, save its HTML and pass the path to the `impeccable_detect` tool.