...
- A skipped detector is a failed critique run unless the `impeccable_detect` tool errors after a real attempt.
- Viewable targets require browser inspection (pi's `web_screenshot` / `web_visit` / `web_console`) when available.
...
2. **Compute the slug**: slugify the resolved path or URL — lowercase, strip the scheme/leading path noise, replace non-alphanumerics with `-`, collapse repeats, trim dashes (e.g. `site/pages/index.astro` → `site-pages-index`, `http://localhost:3000/settings` → `settings`). Keep it for the snapshot filename. If the target is vague or root-level, treat the slug as null: skip persistence and trend for this run, but continue the critique.
...
Run the bundled detector plus rendered-page evidence. Assessment B is mandatory and must remain isolated from Assessment A until both are complete.

Deterministic scan: call the **`impeccable_detect` tool**, passing the markup files/directories (or the saved HTML of a rendered URL) as `targets`.

- Pass markup files/directories; CSS-only files are fine too, but a page's HTML gives the fullest coverage.
- For very large trees (500+ scannable files), narrow scope or ask.
- If the tool errors, report deterministic scan unavailable and continue with rendered/manual review.

Rendered evidence is required for a viewable target when pi's web tools are available:

1. `web_screenshot` the surface (desktop and mobile viewports) for the visual read.
2. `web_visit` (render mode for SPAs) to capture the rendered HTML; save it and pass the path to `impeccable_detect` so the deterministic rules run against the real DOM, not just source.
3. `web_console` to catch runtime errors that affect the experience.
4. For multi-view targets, repeat on 3-5 representative pages.

Return: detector findings/counts, rendered screenshot observations, console findings if applicable, false positives, and skipped steps with concrete reasons.

After Assessment B returns usable detector findings, reuse them. Do not rerun `impeccable_detect` in the parent unless Assessment B failed, was truncated, or omitted count, rule names, or file locations.
...
1. **Write the snapshot file** to `.impeccable/critique/<slug>-<YYYYMMDD-HHMMSS>.md` (create the directory if needed). Start the file with YAML frontmatter, then the full critique body (heuristic table, anti-patterns verdict, priority issues, persona red flags, minor observations, and questions) — but stop before the "Ask the User" / "Recommended Actions" sections that come later.
...
2. **Read the trend** for context: list `.impeccable/critique/<slug>-*.md`, read the `total_score` from the frontmatter of the most recent 5 (including the one you just wrote), oldest → newest.
...
This is fire-and-forget. Failures here should not block the rest of the flow; mention the error briefly and move on.
