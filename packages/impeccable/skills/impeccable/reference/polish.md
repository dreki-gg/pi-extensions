...
4. **Pull in any prior critique** (optional signal): If `/impeccable critique` has been run on the same target, its priority issues are a useful prior for what to address first. Resolve the target to a file path or URL, slugify it the same way critique does, then look under `.impeccable/critique/` for `<slug>-*.md` and read the most recent snapshot. If one exists, fold its P0/P1 items into your polish list and mention the snapshot path so the user sees what you read. If none exists, continue without it. The critique is one input among many. Do your own pass either way. You can also run the `impeccable_detect` tool on the target for a fresh deterministic pass.
