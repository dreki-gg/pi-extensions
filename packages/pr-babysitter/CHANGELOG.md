...
- Add the agent-callable `babysit_pr` tool. After creating or pushing to a PR, the
  ...
  heartbeat keeps the elapsed counter ticking live between polls so a long wait
  stays visibly alive, Esc cancels (including any in-flight `gh` call), and every
  `gh` call is bounded by a per-call timeout so a stalled network call cannot
  freeze the loop. A failed/timed-out poll is retried rather than misread as a
  verdict. While it waits, no LLM tokens are consumed. Configurable via `pr`,
  `pollSeconds` (default 15), `timeoutSeconds` (default 1200), and
  `callTimeoutSeconds` (default 30).
